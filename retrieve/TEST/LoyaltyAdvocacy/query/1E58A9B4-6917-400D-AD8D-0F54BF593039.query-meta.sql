...
WHERE
    (
        Event.Start_Time__c < (GETDATE())
        /*in case this changes --> Event.Start_Time__c < DATEADD(day, +3, GETDATE())*/
        AND Event.Name IN (
            '2023 City of Hope Atlanta Celebrate Life',
            'At-Home Celebration: 2023 City of Hope Atlanta Celebrate Life'
        )
        AND Reg.Status__c = 'Attended'
    )
