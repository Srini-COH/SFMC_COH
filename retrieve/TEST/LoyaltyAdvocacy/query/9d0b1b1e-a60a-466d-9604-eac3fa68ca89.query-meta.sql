SELECT DISTINCT
    ...
    Contacts.How_were_you_affected_by_cancer__c AS HowAffectedByCancer
FROM
...
WHERE
    ...
    AND (
        Contacts.CF_Member_Created_Date__c >= '2020-03-16'
    )
    AND (
        Contacts.CF_Member_Created_Date__c <= DATEADD(DAY, -21, GETDATE())
    )
    ...
    AND AllSubscribers.Status NOT IN ('unsubscribed', 'held')
