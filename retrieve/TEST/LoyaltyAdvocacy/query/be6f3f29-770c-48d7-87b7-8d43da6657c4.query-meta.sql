...
WHERE
    ...
    AND (Contacts.CF_Member_Created_Date__c <= DATEADD(day, -42, GETDATE()))

    AND Contacts.How_were_you_affected_by_cancer__c = 'Patient'
    AND Contacts.ctca_medicalrecordnumber__c IS NOT NULL
    AND Contacts.Affiliated_Facility__c IN ('CTCA, Atlanta','CTCA, Chicago','CTCA, Phoenix','City of Hope Atlanta','City of Hope Chicago','City of Hope Phoenix')
