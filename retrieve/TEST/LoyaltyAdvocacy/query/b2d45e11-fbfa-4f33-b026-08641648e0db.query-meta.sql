...
WHERE
    ...
    AND (
        Contacts.CF_Member_Created_Date__c <= DATEADD(DAY, -180, GETDATE())
    )
    AND (Account.[Celebrant_Eligible__c] NOT LIKE '1')
    AND U.Username IS NOT NULL
    /*Consider 6 months back for CTCA patient but much sooner for non CTCA patients including CGs?)*/
