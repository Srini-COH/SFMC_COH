SELECT DISTINCT
    ...
    ObjLead.Patient_Gender__c AS Gender,
...
FROM
    ...
    LEFT JOIN [LeadJourneyDE_LOG] NurtureLog ON (ObjLead.Id = NurtureLog.LeadId)
WHERE
    (
        ...
        AND (
            (ObjLead.Date_Referred__c IS NULL)
            OR (
                ObjLead.Date_Referred__c <= DATEADD(DAY, -3, GETDATE())
            )
        )
        /*AND (
        ...
        */
        ...
        AND (ObjLead.Pediatric__c NOT LIKE '1')
        AND (
            (Facility.Name IS NULL)
            OR (
                Facility.Name != 'SRMC'
                AND Facility.Name != 'ERMC'
            )
        )
