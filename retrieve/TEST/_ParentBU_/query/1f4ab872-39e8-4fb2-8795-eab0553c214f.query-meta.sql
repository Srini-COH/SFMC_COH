...
WHERE
    (
        ...
        AND ObjLead.Owner_Profile_Name__c IN ('OIS Rep', 'Patient Advocate', 'Facilitator')
        ...
        AND (
            (ObjLead.Previous_Treatment_Type__c IS NULL)
            OR ObjLead.Previous_Treatment_Type__c IN ('None', 'Unknown')
        )
    )
