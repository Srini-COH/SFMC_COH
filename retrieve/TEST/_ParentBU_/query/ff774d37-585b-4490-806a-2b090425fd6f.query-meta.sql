...
WHERE
    (
        ...
        AND (
            (Contacts.Pediatric__c IS NULL)
            OR (Contacts.Pediatric__c NOT LIKE '1')
        )
        ...
        AND (
            (ObjOpportunity.Topics_Sent__c LIKE '%ToolsTx%')
            OR (
                ObjOpportunity.Topics_Sent__c LIKE '%ToolsAndTreatments%'
            )
        )
    )
