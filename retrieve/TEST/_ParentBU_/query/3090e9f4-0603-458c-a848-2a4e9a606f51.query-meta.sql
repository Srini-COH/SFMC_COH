SELECT DISTINCT
    ...
    (
        ...
        WHERE
            (
                ...
                AND (
                    DateSent >= '2019-07-01'
                    AND DateSent < '2020-07-01'
                )
            )
    ) AS LeadNurture_Count_FY20,
    (
        SELECT
            COUNT(TopicName)
        FROM
    ...
    ) AS InsApprovedNurture_Count_FY20,
    (
        SELECT
            COUNT(TopicName)
        FROM
...
WHERE
    (
        ...
        AND JourneyLead.DateSent >= '2019-07-01'
        AND JourneyLead.DateSent <= '2020-06-30'
    )
    OR (
        JourneyOpp.OpportunityId = LEAD.ConvertedOpportunityId
        AND JourneyOpp.DateSent >= '2019-07-01'
        AND JourneyOpp.DateSent <= '2020-06-30'
    )
