SELECT DISTINCT
    ...
    A.NurturedOppCount AS NurturedOppCount
FROM
    [AccountsWithMultipleNurturedOpps] A
    INNER JOIN [Opportunity_Salesforce] ObjOpportunity ON A.AccountID = ObjOpportunity.AccountId
    ...
    AND (ObjOpportunity.Cancer_Type__c NOT LIKE '%Undiagnosed%')
    AND (ObjOpportunity.Cancer_Type__c NOT LIKE '%Screening%')
    ...
    AND (
    ObjOpportunity.ECOG__c NOT IN ('In Hospice', 'Not able to perform activities','In Hospice/Poor ECOG','In Hospital/Critical Care','Primary Site Transplant')
    )
    AND (
    ...
    AND ObjOpportunity.Owner_Profile_Name__c IN ('NPES', 'Patient Advocate', 'Facilitator','Super Advocate Pilot','Advocate Manager','Empowerment Manager','Virtual Service Advocate','Government Service Center')
    /*( AND (
    ...
    AND ((SELECT COUNT(Id) FROM Facility_Visits__c_Salesforce WHERE (ObjOpportunity.Id=Facility_Visits__c_Salesforce.Opportunity__c AND Facility_Visits__c_Salesforce.ctca_apptstatus__c NOT IN ('Return to OIS','Appt Entry Error')) )=0)
    AND (ObjOpportunity.Purpose_of_call__c IS NULL OR ObjOpportunity.Purpose_of_call__c NOT IN ('Non-Oncology'))
    AND (ObjOpportunity.Intake_Type__c is NULL OR ObjOpportunity.Intake_Type__c = 'Oncology')
    AND (Contacts.Pediatric__c NOT LIKE '1')
    AND (ObjOpportunity.Insurance_Status__c IN ('Rapid Approved', 'Full IV Approved'))
    AND (ObjOpportunity.Email__c IS NOT NULL)
    AND (ObjOpportunity.Email__c NOT LIKE '%@dmctca.com%')
    AND ((Facility.Name IS NULL) or (Facility.Name IN ('MRMC','SERMC','WRMC')))
    AND (ObjOpportunity.Cancer_Type__c IS NOT NULL)
    AND (ObjOpportunity.Lead_Score__c >(10) )
    AND (ObjOpportunity.StageName IN ('Insurance Approved') OR ((ObjOpportunity.StageName IN ('Inactive') AND (ObjOpportunity.Last_Stage__c IN ('Insurance Approved')))))
    ...
    AND
    ObjOpportunity.CreatedDate >= DATEADD(day, -180, GETDATE())
    )
    )
    
    AND (ObjOpportunity.COH_Referral__c IS NULL or ObjOpportunity.COH_Referral__c !='Offered/Accepted')
    AND (ObjOpportunity.RecordTypeId = '0121C000001RL6JQAW')
    
    AND (ObjOpportunity.Contact_Status__c IS NULL)
    AND (Contacts.ctca_newstage__c NOT IN ('Expired','Hospice'))
    ...
    AND (Contacts.donotbulkpostalmail__c IS NULL or Contacts.donotbulkpostalmail__c !='Do not Allow')
    AND (Contacts.Nurture_Status__c IS NULL or Contacts.Nurture_Status__c !='Declined')
    AND (Contacts.HasOptedOutOfEmail NOT LIKE '1')
    AND (AllSubscribers.Status IS NULL or AllSubscribers.Status = 'Active')
    AND (ObjOpportunity.Owner_Email__c NOT IN (
    ...
    OR
    (ObjOpportunity.Nurture_Last_Sent__c <= DATEADD(day, -2, GETDATE()))
    )
