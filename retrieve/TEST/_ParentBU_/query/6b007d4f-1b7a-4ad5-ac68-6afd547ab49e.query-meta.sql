...
FROM
    ...
    LEFT JOIN [Facility_Visits__c_Salesforce] FV ON ObjOpportunity.Id = FV.Opportunity__c
    LEFT JOIN [Cancer_Type_Mapping_DE_Journey] CancerMapping ON (
        ObjOpportunity.Cancer_Type__c = CancerMapping.CancerType
    )
WHERE
    ObjOpportunity.Id = '0061C00000WvhjiQAB'
    /*
    (FV.ctca_apptstatus__c LIKE 'Scheduled')
    AND (FV.ctca_apptdate__c >= DATEADD(day, 1, GETDATE()))
    AND (FV.ctca_scheduledon__c <= DATEADD(day, -2, GETDATE()))
    AND (FV.ctca_1sttxdate__c IS NULL)
    AND (FV.ctca_appointmentresults__c IS NULL)
    AND ((FV.Appointment_Type__c IS NULL) OR (FV.Appointment_Type__c NOT LIKE '%Telehealth%'))
    AND ((ObjOpportunity.Intake_Type__c IS NULL) OR (ObjOpportunity.Intake_Type__c != 'Non-Oncology'))
    AND (ObjOpportunity.StageName NOT IN ('Insurance Denied','Closed Lost','Treated','Appointment Kept'))
    AND (ObjOpportunity.Insurance_Category__c NOT LIKE '%7%')
    
    */
    /*AND (ObjOpportunity.Insurance_Category__c NOT LIKE '%3%')*/
    /*
    AND ObjOpportunity.Owner_Profile_Name__c IN ('NPES','Scheduler', 'Patient Advocate', 'Facilitator','Super Advocate Pilot','Advocate Manager','Empowerment Manager','Virtual Service Advocate','Government Service Center')
