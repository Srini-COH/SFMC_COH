...
WHERE
    (
        (FV.ctca_apptstatus__c LIKE 'Scheduled') /*OR ((FV.ctca_apptstatus__c LIKE 'Other') AND (FV.Appointment_Type__c LIKE 'Telehealth'))*/
    )
    AND (FV.ctca_apptdate__c >= DATEADD(DAY, 1, GETDATE()))
    /*AND (FV.ctca_scheduledon__c <= DATEADD(day, -1, GETDATE()))*/
    AND (FV.ctca_1sttxdate__c IS NULL)
    AND (FV.ctca_appointmentresults__c IS NULL)
    AND (
        (FV.Appointment_Type__c IS NULL)
        OR (FV.Appointment_Type__c NOT LIKE '%Telehealth%')
    )
    AND (
